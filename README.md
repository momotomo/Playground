...
このフェーズでは、再編集用 JSON 保存、`PNG 出力`、`ズーム / パン`、`図形再編集`、`ドラッグ矩形選択`、`複数選択の一括リサイズ / 回転`、`Group / Ungroup`、`整列 / 等間隔配置`、`重なり順操作`、`最小レイヤー機能` に加えて、`グリッド / ガイド / スナップ` を追加しています。
...
- 左パネルの `Grid & Guides` から表示とスナップを切り替えます
...
- グリッド表示、ガイド表示、スナップ設定、ガイド位置も同じ `document` 内に保存されます
