...
このフェーズでは、再編集用 JSON 保存、`PNG 出力`、`ズーム / パン`、`図形再編集` に加えて、`複数選択`、`一括移動`、`整列`、`重なり順操作` を追加しています。
...
- 複数選択中は各要素のハイライトとグループバウンディングボックスを表示します
- 単一選択の図形は次の再編集に対応します
  - 移動
  - リサイズ
  - 回転
...
- 複数選択による整列結果や重なり順変更も、そのまま出力へ反映されます
...
- 選択 / 複数選択 / 整列 / 重なり順変更 / 図形再編集 / ズーム / パンの基本操作は native / web で同じです
